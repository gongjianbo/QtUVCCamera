...
// 处理窗口消息
LRESULT CALLBACK CameraHotplugPrivate::windowMessageProcess(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_DEVICECHANGE) {
        do {
            ...
            CameraHotplugPrivate *data = reinterpret_cast<CameraHotplugPrivate *>(::GetWindowLongPtrW(hwnd, GWLP_USERDATA));
            if (!data)
                break;
            // 过滤不监听的设备类型
            ...
            // 从设备描述中获取 vid 和 pid
            quint16 vid, pid;
            if (!CameraInfo::getVidPid(device_name, vid, pid)) {
                break;
            }
            if (is_add) {
...
// 窗口类名
QString CameraHotplugPrivate::windowClassName()
{
    return QLatin1String("Qt_CameraHotplug_Window_") + QString::number(quintptr(windowMessageProcess));
}

// 创建一个用于接收消息的窗口，注册消息回调
bool CameraHotplugPrivate::createMessageWindow(const QVector<QUuid> &uuids)
{
...
// 释放
void CameraHotplugPrivate::destroyMessageWindow()
{
    if (hwnd) {
        ::DestroyWindow(hwnd);
        hwnd = nullptr;
...
CameraHotplug::CameraHotplug(QObject *parent)
    : QObject{parent}
    , dptr{new CameraHotplugPrivate}
{
    dptr->ptr = this;
}

CameraHotplug::~CameraHotplug()
{
    free();
}

void CameraHotplug::init(const QVector<QUuid> &uuids)
{
